...
st.title("🥕 Carrot Price & Weather Analysis")
st.markdown("Explore the relationship between Carrot prices and weather conditions in Hoshiarpur.")

selected_vegetable = "Carrot" # Hardcode for this page
...
if 'Date' in df_selected_veg.columns:
    ...
    df_selected_veg['Day'] = df_selected_veg['Date'].dt.day
...
if not df_selected_veg.empty:
    ...
    default_years_yoy = [year for year in requested_years if year in available_years]

    selected_years_yoy = st.multiselect(
        f"Select Years to Compare Price Distributions for {selected_vegetable}",
        available_years,
        default=default_years_yoy,
    ...
    if selected_years_yoy:
        ...
        fig_yoy_modal_hist = px.histogram(
            ...
            nbins=50,
            opacity=0.7,
            color_discrete_sequence=px.colors.qualitative.Set1 # Strong, distinct palette
        )
        ...
        fig_yoy_min_hist = px.histogram(
            ...
            nbins=50,
            opacity=0.7,
            color_discrete_sequence=px.colors.qualitative.Pastel1 # Softer pastel palette
        )
    ...
    with col_temp:
        # Scatter Plot - Temperature (Pastel points with a hint of vibrancy)
        fig_scatter_temp = px.scatter(
            ...
            color_discrete_sequence=['#87CEEB'] # Pastel Light Blue
        )
    ...
    with col_prcp:
        # Scatter Plot - Precipitation (Slightly different vibrant/pastel blend)
        fig_scatter_prcp = px.scatter(
            ...
            color_discrete_sequence=['#FFA07A'] # Light Salmon (pastel-ish)
        )
    ...
    # Box Plot - Monthly Variation (Pastel with distinct colors per box)
    fig_seasonal = px.box(
        df_selected_veg,
        x='Month',
        y='Modal_Price',
        ...
        color_discrete_sequence=px.colors.qualitative.Pastel # Use a pastel qualitative palette
    )
    fig_seasonal.update_traces(marker_size=4, jitter=0.5)
    fig_seasonal.update_layout(xaxis_title="Month", yaxis_title="Modal Price (Rs./Quintal)")
    st.plotly_chart(fig_seasonal, use_container_width=True)

    # --- Monthly Average Price Bar Chart (already a form of histogram/bar chart) ---
    ...
    # --- Yearly Price and Weather Summary Table ---
    ...
    # --- Price-Weather Correlation Heatmap ---
